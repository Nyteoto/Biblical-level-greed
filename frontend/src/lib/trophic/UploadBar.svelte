...
{#if uploads.count > 0}
	...
	<div
		class="upload-pill fixed bottom-0 left-1/2 z-40 mb-4 flex w-[186px] -translate-x-1/2 flex-col gap-[7px] bg-surface px-3.5 py-2.5 shadow-md"
	...
	>
		...
		<span class="block h-[3px] overflow-hidden bg-neutral-300">
			<span
...
{#if lost > 0}
	<div
		class="upload-pill fixed bottom-0 left-1/2 z-40 mb-4 flex w-[228px] -translate-x-1/2 flex-col gap-2 bg-surface px-3.5 py-2.5 shadow-md"
	...
	>
			...
			{#each uploads.failed.slice(0, 3) as f (f.item.key)}
				{#if f.item.kind === 'image'}
					<img
						src={f.item.preview}
						alt=""
						class="h-7 w-7 shrink-0 object-cover opacity-70"
					/>
				{:else}
					<!-- A clip's preview is a video object URL and an <img> cannot
					     draw it. A stamp saying what it is beats a broken one. -->
					<span
						class="flex h-7 w-7 shrink-0 items-center justify-center bg-neutral-200 font-mono text-[8px] text-neutral-700"
					>
						clip
					</span>
				{/if}
		...
		<div class="flex gap-1.5">
			<button
				type="button"
				class="lift lift-sm flex-1 bg-ground px-2 py-1 text-[11px] font-semibold text-ink shadow-sm"
				onclick={retryFailed}
			>
				try again
			</button>
			<button
				type="button"
				class=" px-2 py-1 text-[11px] text-neutral-600 transition-colors hover:text-ink"
