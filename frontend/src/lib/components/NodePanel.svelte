<script lang="ts">
	...
	// log, no journal, no notes, no completion — one gate text and nothing else.
	// Every win condition in the foundation is written to be *retired*, and a
	// panel full of controls would invite you to manage it instead.
	const bare = $derived(node.kind === 'reminder');
...
>
	...
	{#if bare}
		<!-- The only field a reminder has. Read it; retire it when it is true
		     and boring. -->
		<div class="flex-1 overflow-y-auto p-4">
			<p class="text-[10px] tracking-[0.16em] text-stone-500 uppercase">Gate</p>
			<p class="mt-2 text-[13px] leading-relaxed whitespace-pre-wrap text-stone-300">
				{node.gate}
			</p>
		</div>
	{:else}
		<!-- One pane now. The journal tab moved up to the domain and merged with
		     notes; what is left here is the node's own parameters. -->
		<div class="flex-1 overflow-y-auto p-4">
